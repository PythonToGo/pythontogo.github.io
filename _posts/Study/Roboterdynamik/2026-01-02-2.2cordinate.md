...
## 전체 시스템 구성의 최소좌표
N개의 몸체로 구성된 다물체 시스템을 $6N$개의 좌표로 설명ㅎ나느 것은 Redundant가 발생한다. 로봇은 관절로 연결되어 있어 자유로운 움직임에 제약이 있기 때문. 그래서 우리는 최소좌표(**Minimalkoordinaten** $q$)를 설정한다. 이것은 시스템 구성을 고유하게 설명하는 최소한의 파라미터 집합ㄴ디ㅏ.
